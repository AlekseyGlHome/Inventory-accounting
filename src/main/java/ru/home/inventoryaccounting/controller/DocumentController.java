...
public class DocumentController {
  ...
  public ResponseEntity<DtoResponse<DocumentHeaderDto>> getByAllOrFilter(
    ...
    RequestParametersForDocHeader parameter =
        RequestParameterUtil.getObjectOfRequestParametersOfDocumentHeader(
            offset,
            limit,
            query,
            intervalStart,
  ...
  public ResponseEntity<DtoResponse<DocumentHeaderAndBodyDto>> getById(@PathVariable long id) {
    return ResponseEntity.ok(
        new DtoResponse<>(1L, List.of(documentsHeaderService.getFullById(id))));
  }

  @PutMapping("/{id}")
  public ResponseEntity<DtoResponse<DocumentHeaderAndBodyDto>> update(
      @PathVariable long id, @RequestBody DocumentHeaderRequest request) {
    return ResponseEntity.ok(
        new DtoResponse<>(1L, List.of(documentsHeaderService.update(id, request))));
  }

  @PostMapping()
  public ResponseEntity<DtoResponse<DocumentHeaderAndBodyDto>> add(
  ...
  public ResponseEntity<DtoResponse<DocumentHeaderDto>> getDeleted(
    ...
    RequestParametersForDocHeader parameter =
        RequestParameterUtil.getObjectOfRequestParametersOfDocumentHeader(
            offset, limit, "", null, null, null, null, null, new String[] {"date"}, "ASC");
